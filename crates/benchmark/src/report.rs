...
use crate::tasks::{self, BenchTier};
...
/// Metadata printed into auto-generated Markdown summaries (not the full text report).
#[derive(Debug, Clone)]
pub struct SummaryMeta {
    pub anvil_git_commit: String,
    pub model: String,
    pub provider: String,
    pub tier: BenchTier,
...
impl SummaryMeta {
    pub fn tier_label(tier: BenchTier) -> &'static str {
        match tier {
            BenchTier::Default => "default",
            BenchTier::Hard => "hard",
        }
    }
}

/// Concise, machine-oriented Markdown for `{llm_model, anvil_commit}` capability snapshots.
pub fn generate_summary_markdown(results: &[RunResult], meta: &SummaryMeta) -> String {
    let mut buf = String::new();
    let tier_l = SummaryMeta::tier_label(meta.tier);
    let overall = overall_pass_rate(results);
    let rollups = compute_rollups(results);
    let delta_note = outer_iteration_delta(&rollups).map(|d| {
        format!(
            "- **Outer iter 1 → last (variance only; cold DB each iter)**: Δ pass rate {:+.1} pp; Δ mean agent loops / task {:+.2}.",
            d.pass_rate_delta * 100.0,
            d.mean_session_iterations_delta
        )
    });

    buf.push_str("# Anvil benchmark capability snapshot\n\n");
    buf.push_str("Auto-generated by `anvil-bench --summary-md`.\n\n");
    ...
    buf.push_str(&format!(
        "- **anvil_commit**: `{}`\n",
        meta.anvil_git_commit
    ));
    buf.push_str(&format!(
        "- **suite**: `{}` (tasks: {})\n",
        tier_l,
        rollups.first().map(|r| r.task_count).unwrap_or(0)
    ));
    buf.push_str(&format!(
        "- **outer_iterations**: {}\n",
        meta.outer_iterations
    ));
    buf.push_str(&format!(
        "- **BENCH_OVERALL_PASS_RATE**: {:.4}\n\n",
        overall
    ));

    buf.push_str("## Task mix (by design)\n\n");
    buf.push_str("| Task | Difficulty | Intent |\n");
    buf.push_str("|------|------------|--------|\n");
    for t in tasks::tasks_for_tier(meta.tier) {
        buf.push_str(&format!(
            "| `{}` | {} | bench-defined goal + grader |\n",
            t.name,
            tasks::task_difficulty(t.name)
        ));
    }
    buf.push('\n');

    if meta.outer_iterations >= 2 && !results.is_empty() {
        buf.push_str("## Observed stability (this run)\n\n");
        buf.push_str("| Task | Difficulty | Pass rate | Runs | Med agent loops |\n");
        buf.push_str("|------|------------|-----------|------|-----------------|\n");
        for t in compute_task_stability(results) {
            buf.push_str(&format!(
                "| `{}` | {} | {:.0}% | {} | {} |\n",
                t.task_name,
                tasks::task_difficulty(&t.task_name),
                t.pass_rate * 100.0,
                t.runs,
                t.median_agent_iters
            ));
    ...
    buf.push_str("## Quality read\n\n");
    buf.push_str("- **Across task types**: use the stability table — pass rate near 100% on `easy`, mixed on `medium`/`hard`, flags where the model struggles with tools vs reasoning vs file constraints.\n");
    if let Some(line) = delta_note {
        buf.push_str(&format!("{}\n", line));
    } else {
        buf.push_str("- **Iteration trend**: need `--iterations` ≥ 2 for a first-vs-last outer-iter summary.\n");
    }
    buf.push_str("- **Evolving over bench loops**: outer iterations reset memory each time; sustained improvement requires comparing separate full runs or A/B overlay settings — see the full text report.\n");

    buf
}

fn git_head_full() -> Option<String> {
...
/// Resolve commit for summary headers (`None` → caller may use `"unknown"`).
pub fn resolve_anvil_git_commit() -> Option<String> {
    git_head_full()
}
...
mod tests {
    ...
    #[test]
    fn summary_markdown_includes_task_table() {
        let results = vec![
            sample_run(1, true, 2, 1, false),
            sample_run(1, true, 2, 1, false),
            sample_run(2, false, 3, 2, false),
            sample_run(2, true, 1, 0, false),
        ];
        let meta = SummaryMeta {
            anvil_git_commit: "deadbeef".to_string(),
            model: "glm-4.7-lite".to_string(),
            provider: "ollama".to_string(),
            tier: BenchTier::Default,
            outer_iterations: 2,
        };
        let md = generate_summary_markdown(&results, &meta);
        assert!(md.contains("glm-4.7-lite"));
        assert!(md.contains("deadbeef"));
        assert!(md.contains("tool_call_basic"));
        assert!(md.contains("Outer iter 1 → last"));
    }
}
