...
pub(crate) struct ProviderArgs {
    ...
    /// leaving it unset uses the server's own default. Ignored by hosted providers.
    #[arg(long)]
...
pub(crate) fn resolve(config: &Config, args: &ProviderArgs) -> anyhow::Result<ResolvedProvider> {
    ...
    let provider: Arc<dyn Provider> = match backend.as_str() {
        ...
        "ollama" => {
            build_ollama_provider(config, &model, args.ollama_timeout_secs, args.temperature)
        }
        _ => Arc::new(
...
fn build_ollama_provider(
    ...
    let mut provider = OllamaProvider::with_timeout(
        base_url,
        model,
